...
public class ProCameraPanAndZoomPlus : BasePC2D, IPreMover
{
    ...
    public static string ExtensionName = "Pan And Zoom Plus";
    ...
    CameraTarget[] originalTargets;
    Vector2[] originalTargetInfluences;
    ...
    protected override void OnDisable()
    {
        ...
        for (int i = 0; i < originalTargets.Length; i++)
        {
            originalTargets[i].TargetInfluenceH = originalTargetInfluences[i].x;
            originalTargets[i].TargetInfluenceV = originalTargetInfluences[i].y;
        }

        ProCamera2D.RemoveCameraTarget(_panTarget);

        ProCamera2D.UpdateScreenSize(_initialCamSize, 1f);
    }
    ...
    void Pan(float deltaTime)
    {
        ...
        {
            _panDelta = PanInput;
            panSpeed = DragPanSpeedMultiplier;
        }
...
#else
            _zoomPoint = new Vector3(Screen.width * 0.5f, Screen.height * 0.5f, Mathf.Abs(Vector3D(ProCamera2D.LocalPosition)));
#endif
        }
